...
function extractSummary(content: string): string {
  ...
  console.log('摘要匹配模式:', summaryPattern);
  
  const match = summaryPattern.exec(content);
  console.log('摘要匹配结果:', match);
  
  if (match) {
    const summary = match[1].trim();
    console.log(`成功提取摘要，长度: ${summary.length}`);
    console.log(`摘要预览: ${summary.substring(0, 100)}...`);
    return summary;
  }
...
// 根据分类标题确定类别
function getCategoryByTitle(sectionTitle: string): 'focus' | 'risk' | 'innovation' {
...
function parseNewsItems(content: string): NewsItem[] {
  ...
  console.log('分类标题匹配模式:', sectionPattern);
  console.log('找到的分类标题:', sectionMatches);
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    console.log(`版块内容预览: ${sectionContent.substring(0, 200)}...`);
    
    // 在每个分类中查找新闻条目 - 优化正则表达式
    const newsPattern = /\*\*(\d+)\. 【[^】]*】([^\*]+)\*\*([\s\S]*?)(?=\*\*\d+\.|$)/g;
    console.log(`新闻匹配模式: ${newsPattern}`);
    ...
    while ((match = newsPattern.exec(sectionContent)) !== null) {
      ...
      console.log(`找到新闻 ${newsNumber}: ${title.substring(0, 30)}...`);
      console.log(`内容块预览: ${contentBlock.substring(0, 100)}...`);
      ...
      if (analysisMatch) {
        ...
        console.log(`找到分析内容，长度: ${content.length}`);
      } else {
        ...
        content = contentBlock
          ...
          .replace(/\n\*\s*/g, ' ') // 将多行内容合并
          .trim();
        console.log(`使用整个内容块，长度: ${content.length}`);
      }
      ...
      if (title && content) {
        ...
        console.log(`成功添加新闻 ${newsNumber}, 内容长度: ${content.length}`);
      }
    }
    
    console.log(`版块 "${category}" 共找到 ${matchCount} 个匹配`);
    
    // 如果没有匹配，输出更多调试信息
    if (matchCount === 0) {
      console.log('此版块没有找到任何新闻，输出原始内容:');
...
// 备用解析方法 - 当标准格式解析失败时使用
function parseNewsItemsFallback(content: string): NewsItem[] {
  ...
  for (let patternIndex = 0; patternIndex < newsPatterns.length; patternIndex++) {
    ...
    for (const match of matches) {
      ...
      console.log(`备用方法找到新闻 ${newsNumber}: ${title.substring(0, 30)}...`);
      ...
      if (analysisMatch) {
        content = analysisMatch[1].trim();
        console.log(`找到分析与影响内容，长度: ${content.length}`);
      } else {
        // 如果没有找到"分析与影响"标识，使用整个内容块
        content = contentBlock
          .replace(/^\*\s*/, '') // 移除开头的星号
      ...
      if (title && content) {
        ...
        console.log(`备用方法成功添加新闻 ${newsNumber}`);
      }
    }
...
// 智能识别新闻分类 - 备用方法
function identifyCategory(title: string, content: string): 'focus' | 'risk' | 'innovation' {
...
export function parseContent(rawContent: string): ParsedContent {
  ...
  console.log('- 新闻列表:', news.map(item => ({ title: item.title, content: item.content.substring(0, 50) + '...', category: item.category })));
...
// 手动设置期数
export function setIssueNumber(issue: number): void {
